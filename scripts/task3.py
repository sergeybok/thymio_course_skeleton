...
class BasicThymio:
    ...
    def basic_move(self,state=[1]):
        ...
        while not rospy.is_shutdown() : #and cur_time < start_time+10:
            if state[0] == 1:
                if min(np.mean(self.center_left_buffer), np.mean(self.center_right_buffer)) < 0.04:
                    state[0] = 2
                ...
                while(min(np.mean(self.rear_left_buffer),np.mean(self.rear_right_buffer)) > 0.11):
                    self.velocity_publisher.publish(vel_msg)
                    rospy.loginfo("FIRST loop, left: %.4f right: %.4f"%(np.mean(self.rear_left_buffer),np.mean(self.rear_right_buffer)))
                while(not np.allclose(np.mean(self.rear_left_buffer),np.mean(self.rear_right_buffer),atol=0.007)):
                    self.velocity_publisher.publish(vel_msg)
                    rospy.loginfo("SECOND loop, left: %.4f right: %.4f"%(np.mean(self.rear_left_buffer),np.mean(self.rear_right_buffer)))
