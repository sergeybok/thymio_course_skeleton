...
def cnn_model_fn(features, labels, mode):
    ...
    # Output Tensor Shape: [batch_size, 84, 84, 20]
    ...
    # Input Tensor Shape: [batch_size, 84, 84, 20]
    # Output Tensor Shape: [batch_size, 42, 42, 20]
    pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2)
    print(pool1.shape)
    # Convolutional Layer #2
    # Computes 64 features using a 5x5 filter.
    # Padding is added to preserve width and height.
    # Input Tensor Shape: [batch_size, 42, 42, 20]
    ...
    #    inputs=pool2_flat, units=64, activation=tf.nn.relu)
...
def main(unused_argv):
    ...
    train_data, train_labels = get_data(
        '/home/stud09/training_data/', stacked=4)
    print(train_data.shape, train_labels.shape)
    eval_data, eval_labels = get_data(
        '/home/stud09/test_data/', stacked=4)
    eval_data = tf.cast(eval_data, tf.float64)
    eval_labels = tf.cast(eval_labels, tf.float64)
    print(eval_data.shape, eval_labels.shape)
    # Create the Estimator
    mnist_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir="/home/stud09/robo_cnn")
